...
prolog_load = {"yap": "-l", "swipl": "-s", "pl": "-s"}
...
def process_arguments(commandline=True):
    ...
    if options["morphology"] == "morphisto":
        sys.stderr.write(
            "Warning: deprecated value 'morphisto' for option 'morphology'. Use 'smor' instead.\n"
        )
...
class Parser:
    ...
    def __init__(self, options, timeout=10):
        ...
        try:
            self.morph = pexpect.spawn(
                "fst-infl2", ["-q", options["smor_model"]], echo=False, encoding="utf-8"
            )
            self.morph.delaybeforesend = 0
        except:
            sys.stderr.write(
                "fst-infl2 failed; does model exist? trying fst-infl (in case model is non-compact)\n"
            )
            try:
                self.morph = pexpect.spawn(
                    "fst-infl",
                ...
                sys.stderr.write(
                    "Error: fst-infl2 not found. Please install sfst and smor_model in config.ini points to a valid model.\n"
                )
                sys.exit(1)
        ...
        swipl_version, _ = Popen(
            ["swipl", "--version"], stdout=PIPE, text=True
        ).communicate()
        swipl_version = swipl_version.split()
        try:
            swipl_version = list(
                map(int, swipl_version[swipl_version.index("version") + 1].split("."))
            )
            if swipl_version[0] >= 8 or (
                swipl_version[0] == 7 and swipl_version[1] >= 7
            ):
        ...
        self.prolog_preprocess = pexpect.spawn(
            ...
            [
                "-q",
                "-s",
                os.path.join(root_directory, "preprocessor", "preprocessing.pl"),
            ],
            echo=False,
            encoding="utf-8",
            timeout=timeout,
        ...
        parser_init = (
            ...
            + "assert(returnsentdelim("
            + options["returnsentdelim"]
            + ")),"
            + "retract(nbestmode(_)),"
            + "assert(nbestmode("
            + str(options["nbestmode"])
            + ")),"
            + "retractall(morphology(_)),"
            + "assert(morphology("
            + morphology_mapping[str(options["morphology"])]
            + ")),"
            + "retractall(lemmatisation(_)),"
            + "assert(lemmatisation("
            + morphology_mapping[str(options["morphology"])]
            + ")),"
    ...
    def preprocess(self, sentences):
        ...
        sentences_out.append(
            "\nw('ENDOFDOC','{0}',['._{0}'],'ENDOFDOC').".format(
                self.options["sentdelim"]
            )
        )
        ...
        self.prolog_preprocess.sendline(
            "retractall(gertwol(_,_,_,_,_)),retractall(lemmatisation(_)),retractall(morphology(_)),assert(lemmatisation(smor)),assert(morphology(smor)),retract(sentdelim(_)),assert(sentdelim('"
            + self.options["sentdelim"]
            + "')),start('"
            + morphfile.name
            + "','"
