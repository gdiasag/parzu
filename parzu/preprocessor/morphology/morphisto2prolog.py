...
def create_morph(word, pos, args):
    if pos == "NN":
        morph = "[{gender},{case},{number},_]".format(
            gender=get_repr("gender", args),
            case=get_repr("case", args),
            number=get_repr("number", args),
        )

    elif pos == "NE":
        morph = "[{gender},{case},{number}]".format(
            gender=get_repr("gender", args),
            case=get_repr("case", args),
            number=get_repr("number", args),
        )
        ...
        morph = "[{definiteness},{gender},{case},{number}]".format(
            definiteness=get_repr("definiteness", args),
            gender=get_repr("gender", args),
            case=get_repr("case", args),
            number=get_repr("number", args),
        )
        ...
        morph = "[{gender},{case}]".format(
            gender=get_repr("gender", args), case=get_repr("case", args)
        )
        ...
        morph = "[{person},{number},{case}]".format(
            person=get_repr("person", args),
            case=get_repr("case", args),
            number=get_repr("number", args),
        )

    elif pos == "PPER":
        morph = "[{person},{number},{gender},{case}]".format(
            person=get_repr("person", args),
            gender=get_repr("gender", args),
            case=get_repr("case", args),
            number=get_repr("number", args),
        ...
        morph = "[{grade},{gender},{case},{number},{declension}]".format(
            grade=get_repr("grade", args),
            gender=get_repr("gender", args),
            case=get_repr("case", args),
            number=get_repr("number", args),
            declension=get_repr("declension", args),
        )
        ...
        morph = "[{person},{number},{tense},{aspect}]".format(
            aspect=get_repr("aspect", args),
            tense=get_repr("tense", args),
            number=get_repr("number", args),
            person=get_repr("person", args),
        )
...
def get_true_pos(raw_pos, line):
    ...
    if raw_pos == "V":
        ...
        if line.startswith("<CAP>"):
            line = line[5:]
        if (
            line.startswith("haben")
            or line.startswith("hab<~>en")
            or line.startswith("werden")
            or line.startswith("werd<~>en")
            or line.startswith("sein")
        ):
            pos += "A"
        elif (
            line.startswith("dürfen")
            or line.startswith("dürf<~>en")
            or line.startswith("können")
            or line.startswith("könn<~>en")
            or line.startswith("sollen")
            or line.startswith("soll<~>en")
            or line.startswith("müssen")
            or line.startswith("müss<~>en")
            or line.startswith("mögen")
            or line.startswith("mög<~>en")
            or line.startswith("wollen")
            or line.startswith("woll<~>en")
        ):
            pos += "M"
        else:
            pos += "V"
