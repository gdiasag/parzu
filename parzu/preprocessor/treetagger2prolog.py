...
def format_conversion(line):
    ...
    try:
        word, pos = line.split()
        newline = "w('{0}', '{1}', ['{0}_{1}'], '{0}').".format(
            prolog_escape(word), prolog_escape(pos)
        )
        return word, newline
    except:
        if line == "\n":
            ...
            sys.stderr.write(
                "Error: Line does not have word and POS tag: {0}\n".format(line)
            )
            raise
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        sys.stderr.write(
            "Usage: "
            + sys.argv[0]
            + " temporary_file_for_morphology sentence_delimiter\n"
        )
        sys.exit(1)
