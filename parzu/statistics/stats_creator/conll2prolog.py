...
try:
    ...
    sys.stderr.write(
        "example call: python conll2prolog.py word < tueba6_goldpos.utf8.conll > tueba.pl\n"
    )
    exit()
...
for linenr, line in enumerate(sys.stdin):
    ...
    if len(fields) == 10:
        pos, word, lemma, ctag, tag, morph, head, label = line.strip().split()[
            :8
        ]
    ...
    print(
        "w({0}, {1}, '{2}', '{3}', '{4}', {5}, {6}).".format(
            i, pos, token, tag, label, head, morph
        )
    )
